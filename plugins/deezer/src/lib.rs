...
/// 32 HTTP calls per invocation — cap the fan-out at 30 so a batch
/// fits the call budget with headroom instead of dying
/// `budget-exceeded` with nothing fetched. Refs past the cap are
...
async fn metadata(payload: &Value) -> Result<Value, GuestError> {
    ...
    // Validate every ref before any request — a malformed ref
    // rejects the batch whether it lands inside or past the cap.
    let validated = refs
        .iter()
        .map(|r| deezer_ref(r, &["track", "album", "artist"]))
        .collect::<Result<Vec<_>, _>>()?;
    let mut items = Vec::with_capacity(validated.len().min(METADATA_FETCH_MAX));
    // Deezer has no batch lookup — one request per ref, in input
    // order, capped at METADATA_FETCH_MAX so the fan-out stays inside
    // the host's 32-call invocation budget. A ref whose resource is
    // absent upstream is omitted, the same way itunes drops
    // genuinely-missing ids.
    for (kind, id) in validated.into_iter().take(METADATA_FETCH_MAX) {
...
mod tests {
    ...
    /// The host admits 32 HTTP calls per invocation: a batch bigger
    /// than `METADATA_FETCH_MAX` still resolves, but only the first
    /// 30 refs are fetched — the batch returns the fetched prefix
    /// rather than dying `budget-exceeded` with nothing.
    #[test]
    fn metadata_fanout_caps_at_fetch_max() {
        let refs: Vec<Value> = (1..=35_u64)
            .map(|i| json!({"provider": "deezer", "kind": "track", "id": i.to_string()}))
            .collect();
        let mut out = invoke("catalog.metadata", json!({"refs": refs}));
        for expected in 1..=METADATA_FETCH_MAX {
            assert_eq!(out["kind"], "http_request", "{out}");
            assert!(
                out["payload"]["url"]
                    .as_str()
                    .unwrap_or_default()
                    .ends_with(&format!("/track/{expected}")),
                "{out}"
            );
            out = step(&http_ok(req_id(&out), NO_DATA));
        }
        assert_eq!(out["type"], "done", "{out}");
        assert!(items_of(&out).is_empty());
    }

    /// A malformed ref rejects the batch even when it sits past the
    /// fetch cap — validation runs before any request is issued.
    #[test]
    fn metadata_malformed_ref_past_cap_still_fails() {
        let mut refs: Vec<Value> = (1..=31_u64)
            .map(|i| json!({"provider": "deezer", "kind": "track", "id": i.to_string()}))
            .collect();
        refs.push(json!({"provider": "deezer", "kind": "track", "id": "abc"}));
